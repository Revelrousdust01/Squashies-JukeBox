...
def randVidAnnou():
    playlist = player.playlist(True)
    isAnnouncement = False
    for song in playlist:
        if (song['@uri'].find('/Video%20Announcements/') != -1):
            isAnnouncement = True
...
# Query for queued songs for app
@app.route('/StatisticsApp', methods = ['GET','POST'])
def statisticsApp():
    ...
    if request.method == 'POST':
        ...
        if date1 < date2:
                ...
                dates = []
        for i in rows:
            dates.append({'@songName':i[0], '@patronName':i[1], '@timeStamp':i[2]})
...
def playAnnouncement(announcement):
    ...
    # Play emergency announcement for App
@app.route('/AnnouncementsApp/<announcement>')
def playAppAnnouncement(announcement):
...
@app.route('/AnnouncementsList')
def announcementList():
    videoAnnouncementList = []
    for i in range (len(videoAnnouncements)):
     videoAnnouncementList.append({'@uri':videoAnnouncements[i]})
    return str(videoAnnouncementList)
# Requests
...
#List user to change passwords for app
@app.route('/ChangePassword')
def appChangePassword():
  with sql.connect('database.db') as con:
        cur = con.cursor()
        cur.execute("SELECT uname FROM staff")
        users = cur.fetchall()
        curType = con.cursor()
        curType.execute("SELECT usertype FROM staff")
        userType = curType.fetchall()
        curid = con.cursor()
        curid.execute("SELECT id FROM staff")
        userid = curid.fetchall()
        user = []
        for i in range (len(users)):
         user.append({'@uname':users[i], '@usertype':userType[i], '@id':userid[i]})
        return str(user).replace('(','').replace(')','').replace(',','').replace('} ', '},').replace(" '@", ", '@")

# List users to change passwords from app
@app.route('/Change', methods=['POST'])
def appChangePasswordUser():
        if request.method =='POST':
            uname = request.form["uname"]
            psw2 = request.form["psw2"]

            newpwhash = bcrypt.generate_password_hash(psw2)
            with sql.connect('database.db') as con:
                cur = con.cursor()
                cur.execute('''UPDATE staff SET pwhash = "%s" WHERE uname = "%s" ''' % (newpwhash.decode(),uname))
                con.commit()

            with open('log.txt', "a+") as file:
                stamp = datetime.datetime.now()
                file.write( str(stamp)[:16] + " " + "App" + " changed password for "+ uname + "\n")
        return "Complete!"
...
def listUsers():
 ...
 if request.method == 'GET':
        con = sql.connect("database.db")
        cur = con.cursor()
        cur.execute('SELECT uname, pwhash, userType FROM staff')
        rows = cur.fetchall()
        # checks form data and compares to registered users
        for row in rows:
            if row[0] == request.args['uname']: 
                if bcrypt.check_password_hash(row[1], request.args["psw"]) == True:
                        print(request.args["uname"] + " logged in")
                        with open('log.txt', "a+") as file:
                            stamp = datetime.datetime.now()
                            file.write( str(stamp)[:16] + " " + request.args["uname"] + " logged in from tablet" + "\n")
                        return row[2]
                else:
                        return "Invalid"
        return  "Invalid"
...
def mediaLibrary():
    ...
    time = {"@time": status["root"]["time"], "@state": status["root"]["state"] }
    if len(request.args) == 1 and 'queue' in request.args and request.args['queue'] == "true":
        mediaLibrary = player.playlist(True)
        mediaLibrary.append(time)
        mediaLibrary = json.dumps(mediaLibrary)    
    else:
...
def addSong():
    ...
    queuer = request.args["queuerName"]
    player.add(uri)
    ...
    if "uri" in request.args:
        try:
            ...
            with sql.connect("database.db") as con:
                cur = con.cursor()
                cur.execute("INSERT INTO [queued] (songName, queuer,[timeStamp]) VALUES (?,?,?)", (song, queuer, stamp) )
                con.commit()
        except:
            con.rollback()
            print("Failed to add "+ song +" to DB")
        finally:
            con.close()

        with open('log.txt', "a+") as file:
            stamp = datetime.datetime.now()
            file.write( str(stamp)[:16] + " " + song+ " added to queue by " + queuer + "\n")
